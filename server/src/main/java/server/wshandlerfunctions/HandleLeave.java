        ...
        try {
            ...
            if (game == null) {
                System.err.println(logPrefix + "WARNING: ChessGame object in GameData is NULL for gameID: " +
                        gameID + ". Treating as effectively over.");
                isGameOver = true;
            } else {
            ...
            if (!isGameOver) {
                ...
                try {
                    ...
                    System.err.println(logPrefix + "ERROR during gameService.leaveGame for ACTIVE game "
                            + gameID +
                            " by user '" + username + "': " + e.getMessage());
                    this.helperFunctions.sendError(session, "Error: Failed to update game state on leave - "
                            + e.getMessage());
                }
            } else {
            ...
            System.out.println(logPrefix + "Broadcasting leave notification to others for game "
            ...
            this.helperFunctions.sendError(session, "Error leaving game: Database error during lookup. "
                    + e.getMessage());
        } catch (IOException e) {
            System.err.println(logPrefix + "IOException during message send/broadcast for user '"
                    + (username != null ? username : "UNKNOWN using token ending ..." + authToken)
                    + "', game " + gameID + ": " + e.getMessage());
        } catch (Exception e) {
            System.err.println(logPrefix + "Unexpected error processing leave for game "
                    + gameID + ", user '" + (username != null ? username : "UNKNOWN using token ending ..."
                    + authToken) + "': " + e.getMessage());
            e.printStackTrace(System.err);
            try {
                if (session.isOpen()) {
                    ...
                    System.err.println(logPrefix +
                            "Session was closed, cannot send final unexpected error message to client.");
                }
            } catch (IOException ioex) {
                System.err.println(logPrefix + "IOException trying to send final unexpected error message: "
                        + ioex.getMessage());
            }
        } finally {
            System.out.println(logPrefix + "Finished handle method execution for LEAVE command. GameID: "
                    + gameID + ", AuthToken ending: ..." + authToken);
        }
    }
}